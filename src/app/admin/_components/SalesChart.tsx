...
export function SalesChart({ chartData }: { chartData: ChartData[] }) {
  function getSalesVariation({ chartData }: { chartData: ChartData[] }) {
    ...
    const current = chartData[chartData.length - 1].vendas;
    const previous = chartData[chartData.length - 2].vendas;
    if (previous === 0) {
      return (
        <>
          {current > 0 ? "Aumento de 100%" : "Sem vendas no mês anterior"}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Número de vendas em R$ nos últimos 6 meses
        </CardDescription>
      </CardHeader>
      <CardContent>
